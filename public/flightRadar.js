...
function calculatePosition(departure, destination, progress) {
...
function updatePlanes() {
    ...
    flights.forEach((flight) => {
        ...
        if (elapsedTime >= 0 && elapsedTime <= flightDuration) {
            ...
            const currentPosition = calculatePosition([flight.Departure_Coordinates.lat, flight.Departure_Coordinates.lng], [flight.Destination_Coordinates.lat, flight.Destination_Coordinates.lng], progress);
            // Move the plane
...
flights.forEach((flight) => {
    ...
    // Add plane marker
    const planeMarker = L.marker([flight.Departure_Coordinates.lat, flight.Departure_Coordinates.lng], { icon: planeIcon })
        .addTo(map)
        .on("click", () => {
